	...
	test_cmake_tmp_stream << "project(";
{
    ...
    if(val.type()==typeid(const char *)){
        test_cmake_tmp_stream<<*(std::any_cast<const char *>(&val));
    }else if(val.type()==typeid(std::string)||val.type()==typeid(const std::string)){
        test_cmake_tmp_stream<<*(std::any_cast<const std::string>(&val));
    }
}
...
test_cmake_tmp_stream<<"\n";
	...
	test_cmake_tmp_stream << "# target_link_libraries(${PROJECT_NAME} PRIVATE drogon)\n";
	test_cmake_tmp_stream << "#\n";
